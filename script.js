...
// ===== EduSync Emergency Fix =====

function updateStudentXP() {
    localStorage.setItem("xp", points);
...
function handleFile(input) {
    ...
    const coach = document.getElementById("aiCoachText");
    if (coach) {
        coach.innerText = "تم تحميل المنهج بنجاح. يمكنك الآن توليد بودكاست، بطاقات، خرائط ذهنية واختبار ذكي.";
    }

    const mapTitle = document.getElementById("mapTitle");
    if (mapTitle) mapTitle.innerText = nameF;

    const quizTitle = document.getElementById("quizTitle");
    if (quizTitle) quizTitle.innerText = nameF;
...
function ensureFileUploaded() {
    if (!isFile && !localStorage.getItem("studentFileName")) {
        alert("ارفع مصدر المنهج أولاً");
        return false;
    }
    return true;
}
...
function startPodcast() {
    if (!ensureFileUploaded()) return;

    document.getElementById("aiLoading").style.display = "block";
    document.getElementById("loadingText").innerText = "جاري توليد بودكاست تعليمي...";
...
function generateRealFlashcards() {
    if (!ensureFileUploaded()) return;
    ...
    document.getElementById("loadingText").innerText = "جاري إنشاء بطاقات تعليمية...";
    ...
    setTimeout(() => {
        ...
        flashRes.innerHTML = `
            ...
            <div style="display:grid; gap:14px;">
                ${[
                    ["ما الفكرة الأساسية في EduSync AI؟", "تحويل المنهج إلى تجربة تعلم تفاعلية ذكية."],
                    ["ما فائدة XP؟", "تحفيز الطالب على الاستمرار وإنجاز المهام."],
                    ["ما معنى التوثيق الرقمي؟", "حفظ إنجازات الطالب وبصمتها التعليمية بشكل موثوق."],
                    ["ما فائدة البطاقات؟", "مراجعة سريعة ومركزة للمفاهيم المهمة."]
                ].map((card, i) => `
                    <div class="smart-flash-card" onclick="this.classList.toggle('flipped')">
                        <div class="smart-card-inner">
                            <div class="smart-card-front">
...
function generateRealQuiz() {
    ...
    loading.style.display = "block";
    document.getElementById("loadingText").innerText = "جاري توليد اختبار ذكي...";

    const questions = [
        {
            q: "ما الهدف الأساسي من EduSync AI؟",
            options: ["تحويل التعلم إلى تجربة تفاعلية", "عرض ملفات فقط", "إلغاء دور المحاضر"],
            correct: 0,
            exp: "المنصة تحول المحتوى إلى أدوات تعلم تفاعلية ومحفزة."
        },
        {
            q: "ما فائدة نظام XP؟",
            options: ["زيادة حجم الملفات", "تحفيز الطالب على الاستمرار", "إغلاق المنصة"],
            correct: 1,
            exp: "XP يحفز السلوك التعليمي ويزيد الاستمرارية."
        },
        {
            q: "لماذا نستخدم Blockchain؟",
            options: ["لتوثيق الإنجازات", "لتغيير الألوان", "لتقليل سرعة التطبيق"],
            correct: 0,
            exp: "التوثيق الرقمي يحفظ إنجازات الطالب وبصمتها."
        }
    ];
                    ...
                    <div class="smart-quiz-card">
                        ...
                        <div class="quiz-options">
                            ${item.options.map((op, i) => `
                                <button onclick="
                                    const box=this.closest('.smart-quiz-card');
                                    const buttons=box.querySelectorAll('button');
                                    buttons.forEach(b=>b.disabled=true);
                                    if(${i}===${item.correct}){
                                        this.classList.add('correct');
                                        box.querySelector('.quiz-feedback').innerHTML='<div class=feedback-correct>✅ إجابة صحيحة<p>${item.exp}</p></div>';
                                        addXP(50);
                                    } else {
                                        this.classList.add('wrong');
                                        buttons[${item.correct}].classList.add('correct');
                                        box.querySelector('.quiz-feedback').innerHTML='<div class=feedback-wrong>❌ إجابة غير صحيحة<p>الإجابة الصحيحة: <strong>${item.options[item.correct]}</strong></p><p>${item.exp}</p></div>';
                                    }
                                ">
                                    ${op}
                                </button>
